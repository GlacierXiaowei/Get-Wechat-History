...
class ImageResolver:
    ...
    def get_image_md5(self, local_id, server_id=None):
        """Resolve image MD5 across legacy and current message-resource schemas."""
        ...
        try:
            columns = {
                row[1] for row in conn.execute("PRAGMA table_info(MessageResourceInfo)").fetchall()
            }
            row = None
            if "message_svr_id" in columns and server_id:
                row = conn.execute(
                    "SELECT packed_info FROM MessageResourceInfo WHERE message_svr_id = ? LIMIT 1",
                    (server_id,),
                ).fetchone()
            if row is None and "message_local_id" in columns:
                row = conn.execute(
                    "SELECT packed_info FROM MessageResourceInfo WHERE message_local_id = ? LIMIT 1",
                    (local_id,),
                ).fetchone()
            if row is None and "local_id" in columns:
                row = conn.execute(
                    "SELECT packed_info FROM MessageResourceInfo WHERE local_id = ? LIMIT 1",
                    (local_id,),
                ).fetchone()
            if row and row[0]:
    ...
    def decode_image(self, username, local_id, server_id=None, key_provider=None):
        ...
        """
        # 1. 获取 MD5
        file_md5 = self.get_image_md5(local_id, server_id=server_id)
        if not file_md5:
        ...
        aes_key = None
        xor_key = 0x88
        if is_v2_format(selected) and key_provider is not None:
            aes_key, xor_key = key_provider(selected)
        ...
        if not result_path:
            if is_v2_format(selected) and not aes_key:
                error = "V2 图片 AES 密钥当前不可用；请先在微信中打开该图片，再重试"
            else:
                error = f"无法解密图片文件: {selected}"
            return {'success': False, 'error': error, 'md5': file_md5}
