...
IMAGE_KEY_SCAN_TIMEOUT_SECONDS = 8.0
...
def scan_memory_for_aes_key(pid, ciphertext, max_seconds=IMAGE_KEY_SCAN_TIMEOUT_SECONDS):
    ...
    try:
        ...
        result = _scan_regions(h_process, rw_regions, ciphertext, deadline)
        if result:
            return result
        ...
        result = _scan_regions(h_process, other_regions, ciphertext, deadline)
        if result:
            return result
...
def _scan_regions(h_process, regions, ciphertext, deadline=None):
...
def find_key_for_dat(dat_path):
    """Return a transient (AES key, XOR key) pair for one V2 file, if loaded in WeChat."""
    with open(dat_path, "rb") as handle:
        ...
        if size >= 2:
            ...
            tail = handle.read(2)
    if header[:6] != b"\x07\x08V2\x08\x07" or len(header) < 31:
        return None, 0x88
    ...
    deadline = time.monotonic() + IMAGE_KEY_SCAN_TIMEOUT_SECONDS
    for pid in get_wechat_pids():
        remaining = deadline - time.monotonic()
        if remaining <= 0:
            break
        key = scan_memory_for_aes_key(pid, ciphertext, max_seconds=remaining)
        if key:
            return key, xor_key
    return None, xor_key
