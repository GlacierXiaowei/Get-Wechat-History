...
# The plugin injects these paths after detecting the active local account.
# Importing this module must never depend on a project-local config file.
...
def configure_reader(db_dir, keys_file, decrypted_dir, decoded_image_dir, cache_dir):
    """Apply plugin-owned paths and reload keys/message shards."""
    global DB_DIR, KEYS_FILE, DECRYPTED_DIR, WECHAT_BASE_DIR, DECODED_IMAGE_DIR
    global ALL_KEYS, MSG_DB_KEYS, _cache, _contact_names, _contact_full
    global _contact_tags, _self_username, _image_resolver

    DB_DIR = os.path.abspath(db_dir)
    KEYS_FILE = os.path.abspath(keys_file)
    DECRYPTED_DIR = os.path.abspath(decrypted_dir)
    DECODED_IMAGE_DIR = os.path.abspath(decoded_image_dir)
    WECHAT_BASE_DIR = os.path.dirname(DB_DIR) if os.path.basename(DB_DIR) == "db_storage" else DB_DIR
    ...
    DBCache.CACHE_DIR = os.path.abspath(cache_dir)
    DBCache.MTIME_FILE = os.path.join(DBCache.CACHE_DIR, "_mtimes.json")
    _cache = DBCache()
    _contact_names = None
    _contact_full = None
    _contact_tags = None
